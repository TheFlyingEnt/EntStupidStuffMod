...
public class BlockGroupFactory {
    ...
    public static void groupStoneFamily(String MainName, Block MainBlock, MapColor MainColor, Boolean Bricks) {
        ...
        Block Wall = BlockFactory.register(MainName + "_wall",
        ...
        if (Bricks) {
            //Chizeled + Cracked
            Block Chiseled = BlockFactory.register(MainName + "_chiseled",
                    (settings) -> new Block(settings),
                    BlockBehaviour.Properties.ofFullCopy(MainBlock)
                );

            Block Cracked = BlockFactory.register("cracked_" + MainName + "s",
                (settings) -> new Block(settings),
                BlockBehaviour.Properties.ofFullCopy(MainBlock)
            );
        }
    ...
    public static void groupFungalFamily(String Varient, MapColor MainColor) {
        Block FUNGAL_PLANKS = BlockFactory.register("fungal" + "_planks" + Varient, Block::new, (BlockBehaviour.Properties.ofFullCopy(Blocks.OAK_PLANKS).mapColor((MainColor))));
