...
public class HatMenuScreen extends Screen {
    ...
    // Pending tooltip — set during the scissored list loop, drawn after disableScissor()
    private List<Component> pendingTooltip  = null;
    private int             pendingTooltipX = 0;
    private int             pendingTooltipY = 0;
 
    private EditBox searchBox;
    private int panelX, panelY, listTop;
 
    public HatMenuScreen() {
        super(Component.literal("Hat Menu"));
    }
    ...
    protected void init() {
        ...
        searchBox = new EditBox(font, listX, searchY, LIST_WIDTH - 2, SEARCH_HEIGHT, Component.literal("Search..."));
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float delta) {
        ...
        g.drawString(font, Component.literal("Hats"), panelX + PREVIEW_WIDTH + 6, panelY + 6, COLOR_GRAY);
 
        renderPlayerPreview(g, mouseX, mouseY);
        renderRemoveButton(g, mouseX, mouseY);
 
        // Clear pending tooltip before each frame
        pendingTooltip  = null;
        renderHatList(g, mouseX, mouseY);
 
        // Draw tooltip AFTER scissor is disabled so it isn't clipped
        if (pendingTooltip != null) {
            g.renderTooltip(
                font,
                pendingTooltip.stream()
    ...
    private void renderHatList(GuiGraphics g, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < ROWS_VISIBLE; i++) {
            ...
            boolean unlocked   = ClientUnlockState.canWear(hatName);
            ...
            if (isEquipped) {
                ...
                g.fill(listX, ry, listX + listW, ry + ROW_HEIGHT, 0xFF1A1A1A);
            } else if (rowHovered) {
            ...
            // Item icon + lock overlay
            var item = HatRegistry.getHat(hatName);
            if (item != null) {
                g.renderItem(new ItemStack(item), listX + 3, ry + 3);
                if (!unlocked) {
                    g.fill(listX + 3, ry + 3, listX + 19, ry + 19, 0xBB000000);
                    g.drawCenteredString(font, Component.literal("\uD83D\uDD12"), listX + 11, ry + 8, COLOR_WHITE);
                }
            }
 
            // Hat name (truncated to leave room for badge)
            HatSource source = HatRegistry.getSource(hatName);
            int badgeW = source.requiresUnlock() ? font.width(source.badge()) + 4 : 0;
            int nameMaxW = listW - 24 - badgeW - 2;
 
            Component name = Component.translatable("item." + net.ent.entstupidstuff.EntStupidStuff.MOD_ID + "." + hatName);
            int textColor = isEquipped ? COLOR_GREEN : (unlocked ? COLOR_WHITE : COLOR_LOCKED);
 
            // Clip name text width manually by scissor-safe drawing
            g.drawString(font, font.plainSubstrByWidth(name.getString(), nameMaxW), listX + 24, ry + 7, textColor);
 
            // Source badge — right-aligned, only for non-default hats
            if (source.requiresUnlock()) {
                int badgeX = listX + listW - font.width(source.badge()) - 2;
                g.drawString(font, source.badge(), badgeX, ry + 7, source.color());
            }
 
            // Queue tooltip for locked hats — rendered AFTER disableScissor()
            if (rowHovered && !unlocked) {
                pendingTooltip  = List.of(
                    Component.literal("Locked").withStyle(ChatFormatting.RED),
                    Component.literal("Source: ").withStyle(ChatFormatting.GRAY)
                        .append(source.displayName().copy().withStyle(ChatFormatting.WHITE))
                );
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean bl) {
        ...
        if (hoveredRow >= 0 && hoveredRow < filteredHats.size()) {
            String hatName = filteredHats.get(hoveredRow);
            if (ClientUnlockState.canWear(hatName)) selectHat(hatName);
            return true;
        }
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
        if (scrolling) { updateScrollFromMouse((int) event.y()); return true; }
