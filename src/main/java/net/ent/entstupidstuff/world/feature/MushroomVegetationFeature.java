...
public class MushroomVegetationFeature extends Feature<NetherForestVegetationConfig> {
	public MushroomVegetationFeature(Codec<NetherForestVegetationConfig> codec) {
		super(codec);
	}
	...
	public boolean place(FeaturePlaceContext<NetherForestVegetationConfig> featurePlaceContext) {
		...
		if (!blockState.is(BlockFactory.SHROOMIUM_BLOCK)) {
			return false;
		} else {
			int i = blockPos.getY();
