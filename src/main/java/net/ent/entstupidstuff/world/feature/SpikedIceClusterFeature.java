...
public class SpikedIceClusterFeature extends Feature<DripstoneClusterFeatureConfig> {
	public SpikedIceClusterFeature(Codec<DripstoneClusterFeatureConfig> codec) {
		super(codec);
	}
	...
	private void generate(
		...
		if (!optional.isEmpty()) {
			...
			if (!optionalInt.isEmpty() || !optionalInt2.isEmpty()) {
				...
				if (optionalInt.isPresent()) {
					SpikedIceHelper.generatePointedDripstone(world, pos.withY(optionalInt.getAsInt() - 1), Direction.DOWN, m, bl4);
				}

				if (optionalInt3.isPresent()) {
					SpikedIceHelper.generatePointedDripstone(world, pos.withY(optionalInt3.getAsInt() + 1), Direction.UP, t, bl4);
				}
			}
		}
	}
